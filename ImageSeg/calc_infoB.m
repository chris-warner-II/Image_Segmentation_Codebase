function Io = calc_infoB(img_seg, groundTruth) %, debug)

% Syntax: I = calc_infoB(img_seg, truth, debug);
%
%

    
    for j = 1:size(groundTruth{1},3)
    
        truth = groundTruth{1}(:,:,j);
        ...
        Is(isnan(Is))=0;                       % (zero if all pixels in one segment & max if Pa = Pb = 0.5)
...
%         figure, subplot(211),imagesc(img_seg),subplot(212),imagesc(groundTruth{1}(:,:,j))
%         keyboard
