...
rM      = {'3'      ,'1',    '1',    '10',        '10'     }; % optimal parameters for each method
...
for j = 1:numel(methods)
    for i = 1:50 %numel(im_ptchs)
        ...
        fid = fopen(['scripts4cluster/script_boundaryGradientMetric_Eig',num2str(c)], 'w+');
        ...
        fprintf(fid,['\n']);
        fprintf(fid,['compare_methods_boundaryGradientMetric_Eig(''',methods{j},''',''',im_ptchs(i).name(1:end-4),''',''',rM{j},''');\n']);
        fprintf(fid,['\n']);
        fprintf(fid,['exit\n']);
        fprintf(fid,['EOF\n']);
        fclose(fid);
...
fid = fopen(['scripts4cluster/Run_scripts_boundaryGradientMetric_Eig'], 'w+');
for k=1:c
    fprintf(fid,['sbatch script_boundaryGradientMetric_Eig',num2str(k),'\n']);
    fprintf(fid,['sleep 1 \n']);
end
...
fid = fopen(['scripts4cluster/View_outs_boundaryGradientMetric_Eig'], 'w+');
for k=1:c
    fprintf(fid,['echo --------------------------------------------------------------------------\n']);
...
fid = fopen(['scripts4cluster/View_errs_boundaryGradientMetric_Eig'], 'w+');
for k=1:c
    fprintf(fid,['echo --------------------------------------------------------------------------\n']);
    fprintf(fid,['echo bGMe',num2str(c),'.err\n']);
    fprintf(fid,['tail -n 20 bGMe',num2str(c),'.err\n']);
    fprintf(fid,['\n']);
end
