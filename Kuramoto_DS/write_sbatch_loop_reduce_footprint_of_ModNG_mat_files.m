...
for i = 1:numel(rM)
    for j = 1:numel(ks)
            ...
            fid = fopen(['scripts4cluster/script_reduce_footprint_of_ModNG_mat_files',num2str(C)], 'w+');

            fprintf(fid,['#!/bin/bash\n']);
            fprintf(fid,['# Job name:\n']);
            fprintf(fid,['#SBATCH --job-name=rNG',num2str(C),'\n']);
            fprintf(fid,['#\n']);
            ...
            fprintf(fid,['#\n']);
            fprintf(fid,['#\n']);
            fprintf(fid,['#SBATCH -o rNG',num2str(C),'.out\n']);
            fprintf(fid,['#\n']);
            fprintf(fid,['#SBATCH -e rNG',num2str(C),'.err\n']);
            fprintf(fid,['\n']);
            %
            fprintf(fid,['\n']);
            fprintf(fid,['module load matlab/R2013a\n']);
            fprintf(fid,['\n']);
            fprintf(fid,['echo "Reducing the size of Mod N&G mat files by deleting weights matrices ',num2str(C),'"\n']);
            fprintf(fid,['date\n']);
            fprintf(fid,['\n']);
            ...
            fprintf(fid,['\n']);
            fprintf(fid,['reduce_footprint_of_ModNG_mat_files(''',rM{i},''',''',ks{j},''',',num2str(blur_flg),');\n']);
            fprintf(fid,['\n']);
            fprintf(fid,['exit\n']);
            fprintf(fid,['EOF\n']);
            fclose(fid);
...
fid = fopen(['scripts4cluster/Run_scripts_reduce_ModNG_mat'], 'w+');
for j=1:C
    fprintf(fid,['sbatch script_reduce_footprint_of_ModNG_mat_files',num2str(j),'\n']);
    fprintf(fid,['sleep 1 \n']);
end
...
fid = fopen(['scripts4cluster/View_outs_reduce_ModNG_mat'], 'w+');
for j=1:C
    fprintf(fid,['echo --------------------------------------------------------------------------\n']);
    fprintf(fid,['echo rNG',num2str(j),'.out\n']);
    fprintf(fid,['tail -n 30 rNG',num2str(j),'.out\n']);
    fprintf(fid,['\n']);
end
...
fid = fopen(['scripts4cluster/View_errs_reduce_ModNG_mat'], 'w+');
for j=1:C
    fprintf(fid,['echo --------------------------------------------------------------------------\n']);
    fprintf(fid,['echo rNG',num2str(j),'.err\n']);
    fprintf(fid,['tail -n 20 rNG',num2str(j),'.err\n']);
    fprintf(fid,['\n']);
end
