function compare_methods_AUC_ROC_res(fileType, fileSize)

    % syntax:  compare_methods_AUC_ROC_res(fileType, fileSize)
    %
    ...
    if(1)
        ...
        for i = 1:numel(rM)
            ...
            AA1 = load([matFilesDir,'AAnrm/AUCdata_AAnrm_rM',rM{i},'_sDInf_sP',sP{i},'_NF_60_0_ks',KS,'.mat']);
            GL1 = load([matFilesDir,'GLnrm/AUCdata_GLnrm_rM',rM{i},'_sDInf_sP',sP{i},'_NF_60_0_ks',KS,'.mat']);
            SK1 = load([matFilesDir,'Mod_SKHAdj/AUCdata_Mod_SKHAdj_rM',rM{i},'_sDInf_sP',sP{i},'_NF_60_0_ks',KS,'.mat']);
%            NG1 = load([matFilesDir,'Mod_N&G/AUCdata_Mod_N&G_rM',rM{i},'_sDInf_sP',sP{i},'_NF_60_0_ks',KS,'.mat']);
            Iso = load([matFilesDir,'IsoDiff/AUCdata_IsoDiff_rM',rM{i},'_NF_60_0_ks',KS,'.mat']);

            
            % Want to Filter these out by Cluster Size - Get rid of Clusters less than 20 pixels.
...
% x2,                
            ...
            % Find Indeces where both clusters are larger than 20 pixels.
            % (Use AA1 - All are same. Verified above.)
            indFilter = 1:numel(AA1.GndTruthID);
            
            %indFilter = ( (AA1.ClusterSizes(1,:)>100) & (AA1.ClusterSizes(2,:)>100) );
            
            %indFilter = (AA1.im_ROC_AUC_1D_accum<0.7);
            
            
            %(AA1.im_ROC_AUC_1D_accum<0.7);
            %( (AA1.im_ROC_AUC_1D_accum>=0.7) & (AA1.im_ROC_AUC_1D_accum<=0.9) );
            %(AA1.im_ROC_AUC_1D_accum>0.9);


            
            AA_diff_KurGS = AA1.kur_ROC_AUC_GS_accum(indFilter) - AA1.im_ROC_AUC_1D_accum(indFilter);
            AA_diff_Kur1D = AA1.kur_ROC_AUC_1D_accum(indFilter) - AA1.im_ROC_AUC_1D_accum(indFilter);
            AA_diff_Ev1 =   AA1.ev1_ROC_AUC_1D_accum(indFilter) - AA1.im_ROC_AUC_1D_accum(indFilter);
            AA_diff_Ev2 =  AA1.ev2o_ROC_AUC_1D_accum(indFilter) - AA1.im_ROC_AUC_1D_accum(indFilter);
            AA_diff_Ev3 =  AA1.ev3o_ROC_AUC_1D_accum(indFilter) - AA1.im_ROC_AUC_1D_accum(indFilter);
            %
            GL_diff_KurGS = GL1.kur_ROC_AUC_GS_accum(indFilter) - GL1.im_ROC_AUC_1D_accum(indFilter);
            GL_diff_Kur1D = GL1.kur_ROC_AUC_1D_accum(indFilter) - GL1.im_ROC_AUC_1D_accum(indFilter);
            GL_diff_Ev1 =   GL1.ev1_ROC_AUC_1D_accum(indFilter) - GL1.im_ROC_AUC_1D_accum(indFilter);
            GL_diff_Ev2 =  GL1.ev2o_ROC_AUC_1D_accum(indFilter) - GL1.im_ROC_AUC_1D_accum(indFilter);
            GL_diff_Ev3 =  GL1.ev3o_ROC_AUC_1D_accum(indFilter) - GL1.im_ROC_AUC_1D_accum(indFilter);
            %
            SK_diff_KurGS = SK1.kur_ROC_AUC_GS_accum(indFilter) - SK1.im_ROC_AUC_1D_accum(indFilter);
            SK_diff_Kur1D = SK1.kur_ROC_AUC_1D_accum(indFilter) - SK1.im_ROC_AUC_1D_accum(indFilter);
            SK_diff_Ev1 =   SK1.ev1_ROC_AUC_1D_accum(indFilter) - SK1.im_ROC_AUC_1D_accum(indFilter);
            SK_diff_Ev2 =  SK1.ev2o_ROC_AUC_1D_accum(indFilter) - SK1.im_ROC_AUC_1D_accum(indFilter);
            SK_diff_Ev3 =  SK1.ev3o_ROC_AUC_1D_accum(indFilter) - SK1.im_ROC_AUC_1D_accum(indFilter);
            %
%             NG_diff_KurGS = NG1.kur_ROC_AUC_GS_accum(indFilter) - NG1.im_ROC_AUC_1D_accum(indFilter);
%             NG_diff_Kur1D = NG1.kur_ROC_AUC_1D_accum(indFilter) - NG1.im_ROC_AUC_1D_accum(indFilter);
%             NG_diff_Ev1 =   NG1.ev1_ROC_AUC_1D_accum(indFilter) - NG1.im_ROC_AUC_1D_accum(indFilter);
%             NG_diff_Ev2 =  NG1.ev2o_ROC_AUC_1D_accum(indFilter) - NG1.im_ROC_AUC_1D_accum(indFilter);
%             NG_diff_Ev3 =  NG1.ev3o_ROC_AUC_1D_accum(indFilter) - NG1.im_ROC_AUC_1D_accum(indFilter);
            %
            ID_diff_KurGS = Iso.kur_ROC_AUC_GS_accum(indFilter) - Iso.im_ROC_AUC_1D_accum(indFilter);
            ID_diff_Kur1D = Iso.kur_ROC_AUC_1D_accum(indFilter) - Iso.im_ROC_AUC_1D_accum(indFilter);
            ID_diff_Ev1 =   Iso.ev1_ROC_AUC_1D_accum(indFilter) - Iso.im_ROC_AUC_1D_accum(indFilter);
            ID_diff_Ev2 =  Iso.ev2o_ROC_AUC_1D_accum(indFilter) - Iso.im_ROC_AUC_1D_accum(indFilter);
            ID_diff_Ev3 =  Iso.ev3o_ROC_AUC_1D_accum(indFilter) - Iso.im_ROC_AUC_1D_accum(indFilter);
...
%             text(1.3, 1.1*(AUC_mn(4,1)+AUC_std(4,1)), num2str(NG1.count_isnan_kur_GS))
            ...
            saveGoodImg(h1,[imgFilesDir,'Comp_NetSeg_Methods_rM',rM{i},'_sP',sP{i}],sizeGoodIm)
            close(h1)
...
% 'N&G',            
            ...
            set(gca,'XTick',[1,2,3,4,5],'XTickLabel',{'Kur UB','Kur 1D','Ev1','Ev2','Ev3'},'FontSize',16,'FontWeight','Bold')
            saveGoodImg(h2,[imgFilesDir,'Comp_NetSeg_Methods_rM',rM{i},'_sP',sP{i},'_filt100pix'],sizeGoodIm)
            close(h2)
