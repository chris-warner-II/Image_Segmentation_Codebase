...
!rm scripts4cluster/script_ClusterKurPairROC*
!rm scripts4cluster/Run_scripts_ClusterKurPairROC
!rm scripts4cluster/View_outs_ClusterKurPairROC
!rm scripts4cluster/View_errs_ClusterKurPairROC
    ...
    for i = 1:numel(PTCH)
        ...
        for j = 1:numel(RM)
            for h = 1:numel(SP)
                ...
                fid = fopen(['scripts4cluster/script_ClusterKurPairROC',num2str(k)], 'w+');
                fprintf(fid,['#!/bin/bash\n']);
                fprintf(fid,['# Job name:\n']);
                fprintf(fid,['#SBATCH --job-name=KROC',num2str(k),'\n']);
                fprintf(fid,['#\n']);
                ...
                fprintf(fid,['#\n']);
                fprintf(fid,['#\n']);
                fprintf(fid,['#SBATCH -o KROC',num2str(k),'.out\n']);
                fprintf(fid,['#\n']);
                fprintf(fid,['#SBATCH -e KROC',num2str(k),'.err\n']);
                fprintf(fid,['\n']);
                fprintf(fid,['module load matlab/R2013a\n']);
                fprintf(fid,['\n']);
                fprintf(fid,['echo Computing Kuramoto Cluster Pairwise ROC Area under Curve for all files',num2str(k),'\n']);
                fprintf(fid,['\n']);
                ...
                fprintf(fid,['\n']);
                fprintf(fid,['compute_Kur_clusterPairROC_fixFiles(''',InputImages,''',''',MethodType{g},''',''',PTCH{i},''',''',RM{j},''',''',SP{h},''');\n']);
                fprintf(fid,['\n']);
                fprintf(fid,['exit\n']);
                fprintf(fid,['EOF\n']);
                fclose(fid);
...
fid = fopen(['scripts4cluster/Run_scripts_ClusterKurPairROC'], 'w+');
for i=1:k
    fprintf(fid,['sbatch script_ClusterKurPairROC',num2str(i),'\n']);
    fprintf(fid,['sleep 1 \n']);
end
...
fid = fopen(['scripts4cluster/View_outs_ClusterKurPairROC'], 'w+');
for i=1:k
    fprintf(fid,['echo --------------------------------------------------------------------------\n']);
    fprintf(fid,['echo ROC',num2str(i),'.out\n']);
    fprintf(fid,['tail -n 30 ROC',num2str(i),'.out\n']);
    fprintf(fid,['\n']);
end
...
fid = fopen(['scripts4cluster/View_errs_ClusterKurMnNVar'], 'w+');
for i=1:k
    fprintf(fid,['echo --------------------------------------------------------------------------\n']);
    fprintf(fid,['echo ROC',num2str(i),'.err\n']);
    fprintf(fid,['tail -n 20 ROC',num2str(i),'.err\n']);
    fprintf(fid,['\n']);
end
