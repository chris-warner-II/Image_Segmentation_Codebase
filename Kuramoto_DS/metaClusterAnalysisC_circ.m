function [MC] = metaClusterAnalysisC_circ(theta, netParams, kurParams)
...
disp(['Inside metaClusterAnalysis (Circular Variable) function : '])
    ...
    for i = 1:C(S) % loop through clusters
        ...
        if( numel(ind_in)*numel(ind_in) < thresholdNodes2DoBatch )
            ...
            for k = 1:(T/spp)
                ...
                %
                AvgPairwiseDist(i,k+1,1) = mean(x(:)); % avg distance between oscillators within same cluster
            ...
            AvgPairwiseDist(i,1,1) = sum(batchMN.*batchWT)./sum(batchWT);        % weighted straight mean
            ...
            for k = 1:(T/spp)
                ...
                AvgPairwiseDist(i,k+1,1) = sum(batchMN.*batchWT)./sum(batchWT);        % weighted straight mean
        ...
        if( numel(ind_in)*numel(ind_out) < thresholdNodes2DoBatch )
            ...
            for k = 1:(T/spp)
                ...
                %
                AvgPairwiseDist(i,k+1,2) = mean(y(:)); % avg distance between oscillators in different clusters
                %
            end
            ...
            AvgPairwiseDist(i,1,2) = sum(batchMN.*batchWT)./sum(batchWT);        % weighted straight mean
            ...
            for k = 1:(T/spp)
                ...
                AvgPairwiseDist(i,k+1,2) = sum(batchMN.*batchWT)./sum(batchWT);        % weighted straight mean
