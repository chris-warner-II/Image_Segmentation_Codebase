...
for i = 1:numel(method)
    for j = 1:numel(RM)
        for k = 1:numel(SP)
            ...
            fid = fopen(['scripts4cluster/script_boundaryGradientMetricEigFF',num2str(C)], 'w+');

            fprintf(fid,['#!/bin/bash\n']);
            fprintf(fid,['# Job name:\n']);
            fprintf(fid,['#SBATCH --job-name=bGMEf',num2str(C),'\n']);
            fprintf(fid,['#\n']);
            ...
            fprintf(fid,['#\n']);
            fprintf(fid,['#\n']);
            fprintf(fid,['#SBATCH -o bGMEf',num2str(C),'.out\n']);
            fprintf(fid,['#\n']);
            fprintf(fid,['#SBATCH -e bGMEf',num2str(C),'.err\n']);
            fprintf(fid,['\n']);
            %
            fprintf(fid,['\n']);
            ...
            fprintf(fid,['\n']);
            fprintf(fid,['compute_Eig_bDgradMetric_fixFiles(''BSDS_patch'',''101x101_ds1'',''',method{i},''',''',RM{j},''',''',SP{k},''');\n']);
            fprintf(fid,['\n']);
            fprintf(fid,['exit\n']);
            fprintf(fid,['EOF\n']);
            fclose(fid);
...
fid = fopen(['scripts4cluster/Run_scripts_boundaryGradientMetricEigFF'], 'w+');
for j=1:C
    fprintf(fid,['sbatch script_boundaryGradientMetricEigFF',num2str(j),'\n']);
    fprintf(fid,['sleep 1 \n']);
end
...
fid = fopen(['scripts4cluster/View_outs_boundaryGradientMetricEigFF'], 'w+');
for j=1:C
    fprintf(fid,['echo --------------------------------------------------------------------------\n']);
    fprintf(fid,['echo bGMEf',num2str(j),'.out\n']);
    fprintf(fid,['tail -n 30 bGMEf',num2str(j),'.out\n']);
    fprintf(fid,['\n']);
end
...
fid = fopen(['scripts4cluster/View_errs_boundaryGradientMetricEigFF'], 'w+');
for j=1:C
    fprintf(fid,['echo --------------------------------------------------------------------------\n']);
    fprintf(fid,['echo bGMEf',num2str(j),'.err\n']);
    fprintf(fid,['tail -n 20 bGMEf',num2str(j),'.err\n']);
    fprintf(fid,['\n']);
end
