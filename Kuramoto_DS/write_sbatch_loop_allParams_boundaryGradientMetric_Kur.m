...
methods = {'Mod_N&G',}; % 'IsoDiff','AAnrm','GLnrm','Mod_SKHAdj',
...
for j = 1:numel(methods)
    for i = 1:50 %numel(im_ptchs)
        ...
        fid = fopen(['scripts4cluster/script_allParams_boundaryGradientMetric_Kur',num2str(c)], 'w+');

        fprintf(fid,['#!/bin/bash\n']);
        fprintf(fid,['# Job name:\n']);
        fprintf(fid,['#SBATCH --job-name=bGMk',num2str(c),'\n']);
        fprintf(fid,['#\n']);
        ...
        fprintf(fid,['#\n']);
        fprintf(fid,['#\n']);
        fprintf(fid,['#SBATCH -o bGMk',num2str(c),'.out\n']);
        fprintf(fid,['#\n']);
        fprintf(fid,['#SBATCH -e bGMk',num2str(c),'.err\n']);
        fprintf(fid,['\n']);
        %
        fprintf(fid,['\n']);
        ...
        fprintf(fid,['\n']);
        fprintf(fid,['compare_allParams_boundaryGradientMetric_Kur(''',methods{j},''',''',im_ptchs(i).name(1:end-4),''');\n']);
        fprintf(fid,['\n']);
        fprintf(fid,['exit\n']);
        fprintf(fid,['EOF\n']);
        fclose(fid);
...
fid = fopen(['scripts4cluster/Run_scripts_allParams_boundaryGradientMetric_Kur'], 'w+');
for k=1:c
    fprintf(fid,['sbatch script_allParams_boundaryGradientMetric_Kur',num2str(k),'\n']);
    fprintf(fid,['sleep 1 \n']);
end
...
fid = fopen(['scripts4cluster/View_outs_allParams_boundaryGradientMetric_Kur'], 'w+');
for k=1:c
    fprintf(fid,['echo --------------------------------------------------------------------------\n']);
    fprintf(fid,['echo bGMk',num2str(k),'.out\n']);
    fprintf(fid,['tail -n 30 bGMk',num2str(k),'.out\n']);
    fprintf(fid,['\n']);
end
...
fid = fopen(['scripts4cluster/View_errs_allParams_boundaryGradientMetric_Kur'], 'w+');
for k=1:c
    fprintf(fid,['echo --------------------------------------------------------------------------\n']);
    fprintf(fid,['echo bGMk',num2str(k),'.err\n']);
    fprintf(fid,['tail -n 20 bGMk',num2str(k),'.err\n']);
    fprintf(fid,['\n']);
end
